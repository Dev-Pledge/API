...
class CreateSolutionHandler extends AbstractCommandHandler {
	...
	protected function handle( $command ) {
		...
		if ( ! ( isset( $data->open_source_location ) && strpos( $data->open_source_location, 'http' ) !== false ) ) {
			throw new InvalidArgumentException( 'Open Source Location i.e GitHub Repository Url is Required', 'open_source_location' );
		}
		...
		if ( ! ( isset( $data->name ) && strlen( $data->name ) > 3 ) ) {
			throw new InvalidArgumentException( 'Please give your Software Solution a name with more than Characters', 'name' );
		}
		...
		if ( $solutionService->getProblemSolutionWithOpenSourceLocation( $problem, $data->open_source_location ) !== null ) {
			throw new InvalidArgumentException( 'This Open Source Location is already being used for a solution on this problem', 'open_source_location' );
		}
